...
export const apiLimiter = rateLimit({
  ...
  max: process.env.NODE_ENV === 'development' ? 10000 : 1000, // Limit each IP to 1000 requests per `window` (here, per 15 minutes)
...
export const authLimiter = rateLimit({
  ...
  max: process.env.NODE_ENV === 'development' ? 1000 : 500, // Limit each IP to 500 requests per `window` (here, per 1 hour)

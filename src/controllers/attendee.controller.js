...
export async function getAttendees(req, res) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Unable to fetch attendees';
...
export async function getAttendeeById(req, res) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Unable to fetch attendee';
    return res.status(500).json({ error: message });
  }
}
...
export async function updateAttendee(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Attendee updated successfully', attendee });
  } catch (error) {
    ...
    const message =
      error instanceof Error ? error.message : 'Unable to update attendee';
    return res.status(400).json({ error: message });
  }
}
...
export async function deleteAttendee(req, res) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Unable to delete attendee';
    return res.status(500).json({ error: message });
  }
}
...
export async function checkInAttendee(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Attendee checked in successfully', attendee });
  } catch (error) {
    if (error instanceof Error && error.message === 'Attendee not found') {
      return res.status(404).json({ error: error.message });
    }
...
export async function checkInAttendeeByQr(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Attendee checked in via QR successfully', attendee });
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : 'Unable to check in attendee via QR';
    return res.status(400).json({ error: message });
  }
}
...
export async function getAttendeesByEventId(req, res) {
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Unable to fetch attendees for event';
    return res.status(500).json({ error: message });
  }
}
